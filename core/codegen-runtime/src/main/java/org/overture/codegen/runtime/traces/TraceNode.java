	...
	public static void executeTests(TraceNode trace, Class instanceType, TestAccumulator acc, Store store)
	{
		try
		{
			...
			for(CallSequence test : tests)
			{
				...
				/*
				 * Type check missing here
				 * 
				 * If the type check fails we would also have to do filtering
				 */
				...
				else
				{
					...
					for (; callStmIdx < test.size(); callStmIdx++)
					{
						...
						} catch (RuntimeException e)
						{
							...
							boolean preCondViolation = e.getMessage() != null && e.getMessage().contains("Precondition failure");
							...
							for (; callStmIdx < test.size(); callStmIdx++)
							{
								...
								{
									callStmResults.add(e.getClass().getSimpleName());// Happens for null pointer exceptions
								}
							}
					...
					if (!failureOccured)
					{
						// Filter (todo: but filtering a passed test should be useless)
