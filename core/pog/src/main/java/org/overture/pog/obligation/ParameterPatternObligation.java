...
public class ParameterPatternObligation extends ProofObligation
{
			...
			throws AnalysisException
	{
		...
		PExp pred = generate(def.getPredef(), cloneListPatternList(def.getParamPatternList()), cloneListType(((AFunctionType) def.getType()).getParameters()), ((AFunctionType) def.getType()).getResult().clone());
		stitch = pred;
			...
			throws AnalysisException
	{
		...
		this.assistantFactory = af;
		// valuetree.setContext(ctxt.getContextNodeList());
		PExp pred = generate(def.getPredef(), cloneListPatternList(assistantFactory.createAImplicitFunctionDefinitionAssistant().getParamPatternList(def)), cloneListType(((AFunctionType) def.getType()).getParameters()), ((AFunctionType) def.getType()).getResult().clone());
		stitch = pred;
		valuetree.setPredicate(ctxt.getPredWithContext(pred));
	}
			...
			throws AnalysisException
	{
		...
		this.assistantFactory = af;
		// valuetree.setContext(ctxt.getContextNodeList());
		PExp pred = generate(def.getPredef(), cloneListPatternList(assistantFactory.createAExplicitOperationDefinitionAssistant().getParamPatternList(def)), cloneListType(((AOperationType) def.getType()).getParameters()), ((AOperationType) def.getType()).getResult().clone());
		stitch = pred;
		valuetree.setPredicate(ctxt.getPredWithContext(pred));
	}
			...
			throws AnalysisException
	{
		...
		this.assistantFactory = af;
		// valuetree.setContext(ctxt.getContextNodeList());
		PExp pred = generate(def.getPredef(), cloneListPatternList(assistantFactory.createAImplicitOperationDefinitionAssistant().getListParamPatternList(def)), cloneListType(((AOperationType) def.getType()).getParameters()), ((AOperationType) def.getType()).getResult().clone());
		stitch = pred;
		valuetree.setPredicate(ctxt.getPredWithContext(pred));
	}
